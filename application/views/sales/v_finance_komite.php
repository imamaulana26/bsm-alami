									...
									foreach ($data as $dt) : ?>
										<tr>
											...
											<td class="text-center">
												<a href="javascript:void(0)" onclick="komite('<?= $dt['kd_invoice'] ?>')"><?= $dt['status']; ?></a>
											</td>
										</tr>
									<?php endforeach; ?>
								</tbody>
...
<div class="modal fade" id="komiteModal" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog modal-xl">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">Catatan Komite</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<form method="POST" action="<?= site_url('sales/finance_komite/submit_komite') ?>">
				<div class="modal-body">
					<input type="hidden" name="inv_komite" id="inv_komite" class="form-control">
					<p class="mb-3"><b>Pengusul</b></p>
					<div class="form-group row">
						<label for="inputBBRM" class="col-sm-2 col-form-label">Jr. BBRM / BBRM</label>
						<div class="col-sm-3">
							<input type="text" name="inputBBRM" id="inputBBRM" class="form-control" readonly>
						</div>
					</div>
					<div class="form-group row">
						<label for="inputABBM" class="col-sm-2 col-form-label">ABBM</label>
						<div class="col-sm-3">
							<input type="text" name="inputABBM" id="inputABBM" class="form-control" readonly>
						</div>
					</div>
					<div class="form-group row">
						<label for="inputRRO" class="col-sm-2 col-form-label">RRO</label>
						<div class="col-sm-3">
							<input type="text" name="inputRRO" id="inputRRO" class="form-control">
						</div>
					</div>
					<hr>
					<p class="mb-3"><b>Disposisi Pejabat Pemegang Wewenang</b></p>
					<div class="form-group row">
						<label for="inputAM" class="col-sm-2 col-form-label">Area Manager</label>
						<div class="col-sm-3">
							<input type="text" name="inputAM" id="inputAM" class="form-control" readonly>
						</div>
					</div>
					<fieldset class="form-group">
						<div class="row">
							<label class="col-form-label col-sm-2 pt-0">Hasil Komite</label>
							<div class="col-sm-10">
								<div class="form-check">
									<input class="form-check-input" type="radio" name="hsl_komite" id="gridRadios1" value="disetujui">
									<label class="form-check-label" for="gridRadios1" style="cursor: pointer;">
										Disetujui
									</label>
								</div>
								<div class="form-check">
									<input class="form-check-input" type="radio" name="hsl_komite" id="gridRadios2" value="ditolak">
					...
					<div class="form-group row">
						<label class="col-sm-2 col-form-label">Catatan Komite</label>
						<div class="col-sm-10">
							<textarea name="note_komite" id="editor"></textarea>
						</div>
					</div>
				</div>
...
<script>
	function komite(key) {
		$('#komiteModal').modal('show');

		$.ajax({
			url: '<?= site_url('sales/finance_komite/komite/') ?>' + key,
			type: 'post',
			dataType: 'json',
			success: function(data) {
				$('#inv_komite').val(key);
				$('#inputBBRM').val(data.bbrm.nama);
				$('#inputABBM').val(data.abbm.nama);
				$('#inputAM').val('<?= $this->session->userdata('nama') ?>');
			}
		});
	}
